...
public class First_occ_using_z_algo {
	 ...
	 private static int[] calculateZ(char input[]){
	        ...
	        for(int k=1;k<n;k++){ // l and r are the left and right boundaries for zbox
	            ...
	            z[k]=r-l+1;
	                ...
	                // directly copying the prev z's value bcz it doesn't touches the right boudary of z box
	                if(z[k1]<r-k+1){
	                    z[k]=z[k1];
	                }else{// z value of the ele touches the right boundary of curr zbox
	                   
	                //    reducing the left boudary of the zbox to find the further z value for curr ele-k 
	                    l=k;

	                    // l-r is already the valid zbox for curr ele k , so we will check for values >r and extend the zbox
	                  r++;
	...
	return z;
	    ...
	    public static int z_algo(char s[],char p[]){
	        int m=s.length;
	        int n=p.length;
	...
	public static void main(String[] args) {
		System.out.println(strStr("abscadsfav","cads"));
