...
public class LPS_Manacher_algo {
	public String longestPalindrome(String s) {
        char [] input=s.toCharArray();
        int index=0;
    //pre process the input to convert it into type abc -> $a$b$c$ to handle even length case.
...
//	longest palindromic string
	String ans="";
	for(int i=(maxIdx-(max/2));i<=(maxIdx+(max/2));i++) {
if(newInput[i]!='$')
		ans+=newInput[i];
	}
	return ans;
...
return max;
	...
	public static void main(String[] args) {
		lps("babcbaabcbaccba".toCharArray());
